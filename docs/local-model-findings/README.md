...
|---|---|---|---|
| 2026-09-24 | [Claude Code as the harness: L1 as an agent](2026-09-24-claude-code-harness-L1.md) | After a one-line chat-template fix (every request had failed with HTTP 500), the agent passed the hidden check **15 of 15** in both runs (runs 3 and 4), and both ran out of context (73,370 and 65,612 of 65,536 tokens) after finishing the code. A rule against reading whole profile stores moved the cost to verification rather than removing it. n = 2 | Not yet reviewed line by line and not installed; context is the binding limit, and the next agent job needs a verification budget |
