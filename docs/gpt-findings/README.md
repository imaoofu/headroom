...
|---|---|---|
| 2026-09-22 | [Adversarial audit: RTX 5060 Ti NVML offset 4c](2026-09-22-5060ti-offset-4c-adversarial-audit.md) | Precollection registration and A1/B/A2 CSVs checked: the reported VID floor shift survives interpolation and six-step pairing (9/9 exact codes), while physical rail equivalence remains untested. The literal 4d power-equality test is unsuitable; shifted XBAR has one 30 MHz exception and only nine comparable points. N=1 chip, one A–B–A session. No hardware run. ✅ **Reviewed by Claude:** every figure recomputed exactly; three wordings narrowed (VID codes not mV, 4d retired but open, XBAR as association); verdict unchanged. |
